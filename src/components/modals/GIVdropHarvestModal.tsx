...
import { ConfirmedInnerModal, ErrorInnerModal, SubmittedInnerModal } from './ConfirmSubmit'
import {
  CancelButton,
  GIVRate,
  HarvestAllDesc,
...
import { B, IconHelp, brandColors, Caption, IconGIVStream, Lead } from '@giveth/ui-design-system'
...
import { showPendingClaim, showConfirmedClaim, showFailedClaim } from '../toasts/claim'
...
export const GIVdropHarvestModal: FC<IGIVdropHarvestModal> = ({
  ...
  const [givDropAccStream, setGivDropAccStream] = useState<ethers.BigNumber>(constants.Zero)
  ...
  useEffect(() => {
    setGivDropStream(tokenDistroHelper.getStreamPartTokenPerWeek(givdropAmount))
    const amount = new BigNumber(givdropAmount.mul(9).div(10).toString())
    const percent = new BigNumber(tokenDistroHelper.percent / 100)
    const givDropAcc = amount.times(percent).toFixed(0, BigNumber.ROUND_DOWN)
  ...
  return (
    <Modal showModal={showModal} setShowModal={setShowModal}>
      <HarvestAllModalContainer>
        {(claimState === ClaimState.UNKNOWN || claimState === ClaimState.WAITING) && (
          <>
            <HarvestAllModalTitleRow alignItems='center'>
              <HarvestAllModalTitle weight={700}>GIVdrop</HarvestAllModalTitle>
            </HarvestAllModalTitleRow>
                ...
                {givdropAmount && givdropAmount.gt(0) && (
                  <>
                    ...
                    <GIVBoxWithPrice
                      ...
                      price={calcUSD(formatWeiHelper(givdropAmount.div(10)))}
                    />
                    <HelpRow alignItems='center'>
                ...
                {!balances.givback.isZero() && (
                  <>
                    ...
                    <GIVBoxWithPrice
                      ...
                      price={calcUSD(formatWeiHelper(givBackLiquidPart))}
                    />
                    <HelpRow alignItems='center'>
                ...
                {givdropAmount && givdropAmount.gt(0) && (
                  <>
                    ...
                    <GIVBoxWithPrice
                      ...
                      price={calcUSD(formatWeiHelper(givDropAccStream))}
                    />
                  </>
                )}
                <HarvestAllDesc>
                  When you Claim GIV rewards, all liquid GIV allocated to you is sent to your
                  wallet.
                </HarvestAllDesc>
        ...
        {claimState === ClaimState.SUBMITTING && (
          <SubmittedInnerModal title='GIV' walletNetwork={network} txHash={txResp?.hash} />
        )}
        {claimState === ClaimState.CLAIMED && (
          <ConfirmedInnerModal title='GIV' walletNetwork={network} txHash={txResp?.hash} />
        )}
