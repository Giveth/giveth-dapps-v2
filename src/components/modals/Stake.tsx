...
import { neutralColors, Button, OulineButton, H4 } from '@giveth/ui-design-system'
...
import { approveERC20tokenTransfer, stakeTokens, wrapToken } from '@/lib/stakingPool'
...
export const StakeModal: FC<IStakeModalProps> = ({
  ...
  const { title, LM_ADDRESS, POOL_ADDRESS, GARDEN_ADDRESS } = poolStakingConfig
  ...
  return (
    <Modal showModal={showModal} setShowModal={setShowModal}>
      <StakeModalContainer>
        {stakeState !== StakeStates.SUBMITTED && stakeState !== StakeStates.CONFIRMED && (
          <>
            <StakeModalTitle alignItems='center'>
              <StakingPoolImages title={title} />
              <StakeModalTitleText weight={700}>Stake</StakeModalTitleText>
            </StakeModalTitle>
            <InnerModal>
              <AmountInput
                setAmount={setAmount}
                maxAmount={maxAmount}
                poolStakingConfig={poolStakingConfig}
                disabled={!(stakeState === StakeStates.APPROVE || stakeState === StakeStates.STAKE)}
              />
        ...
        {chainId && stakeState === StakeStates.SUBMITTED && (
          <SubmittedInnerModal title={title} walletNetwork={chainId} txHash={txHash} />
        )}
        {chainId && stakeState === StakeStates.CONFIRMED && (
          <ConfirmedInnerModal title={title} walletNetwork={chainId} txHash={txHash} />
        )}
      </StakeModalContainer>
    </Modal>
  )
