...
import { B, brandColors, Caption, IconGIVStream, IconHelp, Lead } from '@giveth/ui-design-system'
...
export const HarvestAllModal: FC<IHarvestAllModalProps> = ({
  ...
  useEffect(() => {
    ...
    setGivBackStream(tokenDistroHelper.getStreamPartTokenPerWeek(balances.givback))
  ...
  useEffect(() => {
    if (network === config.XDAI_NETWORK_NUMBER && !balances.givDropClaimed && account) {
      fetchAirDropClaimData(account).then(claimData => {
        if (claimData) {
          ...
          setGIVdropStream(tokenDistroHelper.getStreamPartTokenPerWeek(givDrop))
        }
      })
    }
  ...
  const onHarvest = () => {
    ...
    if (poolStakingConfig) {
      if (poolStakingConfig.hasOwnProperty('NFT_POSITIONS_MANAGER_ADDRESS')) {
        //NFT Harvest
        claimUnstakeStake(account, library, currentIncentive, stakedPositions)
          .then(res => {
      ...
      claimReward(config.NETWORKS_CONFIG[network]?.TOKEN_DISTRO_ADDRESS, library)
        .then(txResponse => {
          if (txResponse) {
            setState(HarvestStates.SUBMITTED)
            setTxHash(txResponse.hash)
  ...
  return (
    ...
    >
      <>
        {(state === HarvestStates.HARVEST || state === HarvestStates.HARVESTING) &&
          (sum.isZero() ? (
            <HarvestAllModalContainer>
              <HarvestAllModalTitleRow alignItems='center'>
                <HarvestAllModalTitle weight={700}>{title}</HarvestAllModalTitle>
              </HarvestAllModalTitleRow>
              <NothingToHarvest>You have nothing to claim</NothingToHarvest>
              <CancelButton
            ...
            <HarvestAllModalContainer>
              ...
              <StyledScrollbars autoHeight autoHeightMin={'20Vh'} autoHeightMax={'70Vh'}>
                <HarvestBoxes>
                  {poolStakingConfig && (
                    <SPTitle alignItems='center' gap='16px'>
                      <StakingPoolImages title={poolStakingConfig.title} />
                      <div>
                        <StakingPoolLabel weight={900}>{poolStakingConfig.title}</StakingPoolLabel>
                        <StakingPoolSubtitle>{poolStakingConfig.description}</StakingPoolSubtitle>
                      </div>
                    </SPTitle>
                  )}
                  {claimable && claimable.gt(0) && (
                    <>
                      <GIVBoxWithPrice
                        ...
                        price={calcUSD(
                          formatWeiHelper(rewardLiquidPart, config.TOKEN_PRECISION, false)
                        )}
                      />
                      <HelpRow alignItems='center'>
                        <Caption>Added to your GIVstream flowrate</Caption>
                        <IconWithTooltip
                          icon={<IconHelp size={16} color={brandColors.deep[100]} />}
                          direction={'top'}
                        >
                          <TooltipContent>
                            Increase you GIVstream flowrate when you claim liquid rewards!
                          </TooltipContent>
                        </IconWithTooltip>
                      </HelpRow>
                      <RateRow alignItems='center'>
                        <IconGIVStream size={24} />
                        <GIVRate>{formatWeiHelper(rewardStream)}</GIVRate>
                        <Lead>GIV/week</Lead>
                      </RateRow>
                    </>
                  )}
                  {balances.givback.gt(0) && (
                    <>
                      ...
                      <GIVBoxWithPrice
                        ...
                        price={calcUSD(
                          formatWeiHelper(balances.givbackLiquidPart, config.TOKEN_PRECISION, false)
                        )}
                      />
                      <HelpRow alignItems='center'>
                        <Caption>Added to your GIVstream flowrate</Caption>
                      ...
                      <RateRow alignItems='center'>
                        <IconGIVStream size={24} />
                        <GIVRate>{formatWeiHelper(givBackStream)}</GIVRate>
                        <Lead>GIV/week</Lead>
                      </RateRow>
                    </>
                  )}
                  {givDrop.gt(Zero) && (
                    <>
                      ...
                      <GIVBoxWithPrice
                        amount={givDrop}
                        price={calcUSD(formatWeiHelper(givDrop, config.TOKEN_PRECISION, false))}
                      />
                      <HelpRow alignItems='center'>
                        <Caption>Your initial GIVstream flowrate</Caption>
                      </HelpRow>
                      <RateRow alignItems='center'>
                        <IconGIVStream size={24} />
                        <GIVRate>{formatWeiHelper(givDropStream)}</GIVRate>
                        <Lead>GIV/week</Lead>
                      </RateRow>
                    </>
                  )}
                  {!claimableNow.isZero() && (
                    <>
                      ...
                      <GIVBoxWithPrice
                        amount={claimableNow.sub(balances.givbackLiquidPart)}
                        price={calcUSD(
                          formatWeiHelper(claimableNow, config.TOKEN_PRECISION, false)
                        )}
                      />
                    </>
                  )}
                  <HarvestAllDesc>
                    When you harvest GIV rewards, all liquid GIV allocated to you is sent to your
                    wallet.
                  </HarvestAllDesc>
                  ...
                  {state === HarvestStates.HARVESTING && (
                    <HarvestAllPending>
                      <Lottie options={loadingAnimationOptions} height={40} width={40} />
                      &nbsp;HARVEST PENDING
                    </HarvestAllPending>
                  )}
                  <CancelButton
        ...
        {state === HarvestStates.SUBMITTED && (
          <HarvestAllModalContainer>
            <SubmittedInnerModal title={title} walletNetwork={network} txHash={txHash} />
          </HarvestAllModalContainer>
        )}
        {state === HarvestStates.CONFIRMED && (
          <HarvestAllModalContainer>
            <ConfirmedInnerModal title={title} walletNetwork={network} txHash={txHash} />
          </HarvestAllModalContainer>
        )}
      </>
    </Modal>
