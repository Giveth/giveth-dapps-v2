...
import { useWalletName, EWallets, walletsArray, TWalletConnector } from '@/lib/wallet/walletTypes'
...
const WalletModal = ({ showModal, setShowModal, closeParentModal }: IWalletModal) => {
  ...
  const handleSelect = (selected: { connector: TWalletConnector; value: EWallets }) => {
    if (selectedWallet !== selected.value) {
      activate(selected.connector)
        .then(() => {
          window.localStorage.setItem('selectedWallet', selected.value)
