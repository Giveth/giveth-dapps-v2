...
import { ConfirmedInnerModal, SubmittedInnerModal, ErrorInnerModal } from './ConfirmSubmit'
...
export const V3StakeModal: FC<IV3StakeModalProps> = ({
  ...
  const { unstakedPositions, stakedPositions, currentIncentive } = useLiquidityPositions()
  const positions = isUnstakingModal ? stakedPositions : unstakedPositions
  const { title } = poolStakingConfig
  const [stakeStatus, setStakeStatus] = useState<StakeState>(StakeState.UNKNOWN)
  ...
  const [givBackLiquidPart, setGivBackLiquidPart] = useState<BigNumber>(constants.Zero)
  ...
  const handleStakeUnstake = async (tokenId: number) => {
    ...
    const tx = isUnstakingModal
      ? await exit(tokenIdState, account, library, currentIncentive, setStakeStatus)
      : await transfer(tokenId, account, library, currentIncentive, setStakeStatus)
    setTxStatus(tx)
    try {
  ...
  const handleAction = async (tokenId: number) => {
    ...
    const _reward = await getReward(tokenId, uniswapV3StakerContract, currentIncentive.key)

    const liquidReward = tokenDistroHelper.getLiquidPart(_reward)
    const streamPerWeek = tokenDistroHelper.getStreamPartTokenPerWeek(_reward)
    setTokenId(tokenId)
      ...
      <StakeModalContainer>
        {(stakeStatus === StakeState.UNKNOWN ||
          ...
          <StakeModalTitle alignItems='center'>
            ...
            <StakeModalTitleText weight={700}>{title}</StakeModalTitleText>
          </StakeModalTitle>
        )}
        {(stakeStatus === StakeState.UNKNOWN || stakeStatus === StakeState.CONFIRMING) && (
            ...
            <InnerModalPositions>
              {positions.map(position => (
                <V3StakingCard
                  ...
                  handleAction={isUnstakingModal ? handleAction : handleStakeUnstake}
                  isConfirming={stakeStatus === StakeState.CONFIRMING}
                  selectedPosition={position.tokenId === tokenIdState}
                />
              ))}
            </InnerModalPositions>
          </StyledScrollbars>
        )}
        {(stakeStatus === StakeState.UNSTAKING || stakeStatus === StakeState.CONFIRM_UNSTAKE) && (
          <Scrollbars autoHeight autoHeightMin={'20Vh'} autoHeightMax={'70Vh'}>
            <HarvestContainer>
              <HelpRow alignItems='center'>
                ...
                >
                  <TooltipContent>
                    When you unstake an NFT from this pool, you also harvest any corresponding
                    rewards
                  </TooltipContent>
                </IconWithTooltip>
              </HelpRow>
              ...
              <GIVBoxWithPrice amount={claimableNow.sub(givBackLiquidPart)} />
        ...
        <InnerModalStates>
          ...
          {chainId && stakeStatus === StakeState.SUBMITTING && (
            <SubmittedInnerModal title={title} walletNetwork={chainId} txHash={txStatus?.hash} />
          )}
