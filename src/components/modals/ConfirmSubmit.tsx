...
export const SubmittedInnerModal: FC<IConfirmSubmitProps> = ({ title, walletNetwork, txHash }) => {
    ...
    <>
      ...
      {txHash && (
        <BlockExplorerLink
          ...
          size='Big'
        >
          View on {config.NETWORKS_CONFIG[walletNetwork]?.blockExplorerName}
          &nbsp;
...
export const ConfirmedInnerModal: FC<IConfirmSubmitProps> = ({ title, walletNetwork, txHash }) => {
  const { library } = useWeb3React()
...
export const ErrorInnerModal: FC<IErrorProps> = ({ title, walletNetwork, txHash, message }) => {
  return (
    <>
      <Title>{title}</Title>
