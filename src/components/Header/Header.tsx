...
const Header: FC<IHeader> = () => {
  ...
  useEffect(() => {
    const selectedWalletName = window.localStorage.getItem('selectedWallet')
  ...
  return (
    <>
      ...
      >
        <Row>
          <Logo>
            <Image width='48p' height='48px' alt='Giveth logo' src={`/images/logo/logo1.png`} />
          </Logo>
          <SmallHeaderLinks>
            {/* <IconMenu24 /> */}
            <Link href='/' passHref>
        ...
        <HeaderLinks theme={theme}>
          {menuRoutes.map((link, index) => (
            <Link href={link.href} passHref key={index}>
              <HeaderLink size='Big' theme={theme} active={router.route === link.href}>
                {link.title}
              </HeaderLink>
            </Link>
          ))}
        ...
        <Row gap='8px'>
          ...
          <Link href='/terms' passHref>
            <SmallCreateProject
              label=''
              icon={
                <Image src='/images/plus-white.svg' width={16} height={16} alt='create project' />
              }
              linkType={theme === ETheme.Light ? 'primary' : 'secondary'}
            />
          </Link>
          {active && account && chainId ? (
            <>
                ...
                <BalanceButton outline theme={theme}>
                  <HBContainer>
                    <HBBalanceLogo
                      ...
                      height={'24px'}
                    />
                    <HBContent>{formatWeiHelper(balances.balance)}</HBContent>
                  </HBContainer>
              ...
              >
                    ...
                    <WBInfo>
                      ...
                      <WBNetwork>
                        ...
                        {networksParams[chainId]
                          ? networksParams[chainId].nativeCurrency.symbol
                ...
                {showUserMenu && <MenuWallet setShowWalletModal={setShowWalletModal} />}
              </MenuAndButtonContainer>
            </>
          ) : (
            <div>
              <ConnectButton
                buttonType='primary'
                label={isGIVconomyRoute ? 'CONNECT WALLET' : 'SIGN IN'}
                onClick={() => {
                  if (isGIVconomyRoute) {
      ...
      {showSigninModal && (
        <SignInModal showModal={showSigninModal} closeModal={() => setShowSigninModal(false)} />
      )}
    </>
  )
}
