...
export const RewardMenu = () => {
  ...
  const [showWhatIsGIVstreamModal, setShowWhatIsGIVstreamModal] = useState(false)
  ...
  useEffect(() => {
    setGIVstreamLiquidPart(
      tokenDistroHelper.getLiquidPart(allocatedTokens.sub(givbackLiquidPart)).sub(claimed)
    )
    setFlowRateNow(
      tokenDistroHelper.getStreamPartTokenPerWeek(allocatedTokens.sub(givbackLiquidPart))
    )
  ...
  useEffect(() => {
    ...
    if (chainId === config.XDAI_NETWORK_NUMBER) {
      pools = [...config.XDAI_CONFIG.pools, getGivStakingConfig(config.XDAI_CONFIG)]
    } else if (chainId === config.MAINNET_NETWORK_NUMBER) {
      pools = [...config.MAINNET_CONFIG.pools, getGivStakingConfig(config.MAINNET_CONFIG)]
    }
    if (pools) {
      ...
      pools.forEach(pool => {
        ...
        if (unipoolInfo) {
          ...
          _farmRewards = _farmRewards.add(getUserStakeInfo(type, balances, unipoolHelper).earned)
    ...
    <>
      <RewardMenuContainer isMounted={isMounted} theme={theme}>
        ...
        <NetworkRow>
          ...
          <SwithNetwork onClick={() => switchNetworkHandler(chainId)}>Switch network</SwithNetwork>
        </NetworkRow>
        <FlowrateBox theme={theme}>
          <Overline styleType='Small'>GIVStream Flowrate</Overline>
          <FlowrateRow>
            <Image src='/images/icons/thunder.svg' height='16' width='12' alt='Thunder image' />
            <FlowrateAmount>{formatWeiHelper(flowRateNow)}</FlowrateAmount>
          ...
          <a>
            <PartRow theme={theme}>
              <PartInfo>
                ...
                <Row gap='4px'>
                  <PartAmount medium>{formatWeiHelper(givStreamLiquidPart)}</PartAmount>
                  <PartUnit>GIV</PartUnit>
                </Row>
              </PartInfo>
              <Image src='/images/rarrow1.svg' height='32' width='16' alt='Thunder image' />
            </PartRow>
          </a>
        </Link>
          ...
          <a>
            <PartRow theme={theme}>
              <PartInfo>
                ...
                <Row gap='4px'>
                  <PartAmount medium>{formatWeiHelper(farmsLiquidPart)}</PartAmount>
                  <PartUnit>GIV</PartUnit>
                </Row>
              </PartInfo>
              <Image src='/images/rarrow1.svg' height='32' width='16' alt='Thunder image' />
          ...
          <a>
            <PartRow theme={theme}>
              <PartInfo>
                ...
                <Row gap='4px'>
                  <PartAmount medium>{formatWeiHelper(givbackLiquidPart)}</PartAmount>
                  <PartUnit>GIV</PartUnit>
                </Row>
              </PartInfo>
              <Image src='/images/rarrow1.svg' height='32' width='16' alt='Thunder image' />
...
export const FlowrateBox = styled.div`
  background-color: ${props =>
    props.theme === ETheme.Dark ? brandColors.giv[500] : neutralColors.gray[200]};
  margin: 16px 0;
  border-radius: 8px;
  padding: 8px 16px;
`
