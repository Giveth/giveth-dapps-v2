    ...
    <>
      <RewadCardContainer className={className}>
        {!network || !targetNetworks.includes(network) ? (
          <WrongNetworkInnerModal targetNetworks={targetNetworks} text={wrongNetworkText} />
        ) : (
          <>
            <CardHeader justifyContent='space-between'>
              <CardTitle>{title}</CardTitle>
              <ChainInfo alignItems='center'>
                {network === config.MAINNET_NETWORK_NUMBER && <IconEthereum size={16} />}
                {network === config.XDAI_NETWORK_NUMBER && <IconXDAI size={16} />}
                <ChainName styleType='Small'>
                  {network === config.MAINNET_NETWORK_NUMBER ? 'ETH' : 'XDAI'}
                </ChainName>
              </ChainInfo>
            </CardHeader>
