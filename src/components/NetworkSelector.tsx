...
export const NetworkSelector = () => {
  ...
  const handleChangeNetwork = async (networkNumber: number, network: BasicNetworkConfig) => {
  ...
  return (
    <>
      {chainId ? (
        <NetworkSelectorContainer disabled={!supportedNetworks.includes(chainId)}>
          <XDaiSelecor
            isSelected={chainId === config.XDAI_NETWORK_NUMBER}
            onClick={() => {
              handleChangeNetwork(config.XDAI_NETWORK_NUMBER, config.XDAI_CONFIG)
            }}
          >
            <IconXDAI size={24} />
            <B>xDai</B>
          ...
          <EthSelector
            isSelected={
              chainId === config.MAINNET_NETWORK_NUMBER || !supportedNetworks.includes(chainId)
            }
            onClick={() => {
              handleChangeNetwork(config.MAINNET_NETWORK_NUMBER, config.MAINNET_CONFIG)
            }}
          >
