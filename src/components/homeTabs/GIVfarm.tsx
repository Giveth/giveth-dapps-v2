...
export const TabGIVfarmTop = () => {
  ...
  useEffect(() => {
    ...
    setRewardStream(tokenDistroHelper.getStreamPartTokenPerWeek(totalEarned))
      ...
      <TopInnerContainer>
        ...
        <Row justifyContent='space-between'>
          <Left>
            ...
            <Subtitle size='medium'>Stake tokens in the GIVfarm to grow your rewards.</Subtitle>
          </Left>
          <Right>
            <GIVfarmRewardCard
    ...
    <GIVfarmTabContainer>
      ...
      {chainId === config.XDAI_NETWORK_NUMBER && (
        <PoolRow justifyContent='center' gap='24px' wrap={1}>
          {config.XDAI_CONFIG.pools.map((poolStakingConfig, index) => {
            return (
              <StakingPoolCard
          ...
          <StakingPoolCard
            network={config.XDAI_NETWORK_NUMBER}
            poolStakingConfig={getGivStakingConfig(config.XDAI_CONFIG)}
          />
        </PoolRow>
      )}
      {(!chainId ||
        ...
        >
          {config.MAINNET_CONFIG.pools.map((poolStakingConfig, index) => {
            return poolStakingConfig.type === StakingType.UNISWAP ? (
              <StakingPositionCard
          ...
          <StakingPoolCard
            network={config.MAINNET_NETWORK_NUMBER}
            poolStakingConfig={getGivStakingConfig(config.MAINNET_CONFIG)}
          />
        </PoolRow>
      )}
    </GIVfarmTabContainer>
