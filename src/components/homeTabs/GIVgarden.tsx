...
export const TabGardenTop = () => {
  ...
  const [earnedLiquidPart, setEarnedLiquidPart] = useState<ethers.BigNumber>(Zero)
  const [earnedStream, setEarnedStream] = useState<BigNumber>(new BigNumber(0))

  const { earned } = useStakingPool(poolStakingConfig, config.XDAI_NETWORK_NUMBER)
      ...
      <Container>
        ...
        <Row justifyContent='space-between' alignItems='flex-start'>
          <Section1Subtitle size='small'>
            GIV token holders influence the treasury, roadmap and mission of the Giveth ecosystem.
            By voting in the GIVgarden you earn rewards on your staked GIV!
          </Section1Subtitle>
          <OpenGardenButton
            buttonType='primary'
        ...
        <Section2Title weight={500}>Three Pillars of Governance</Section2Title>
        <GovernanceRaw wrap={1} justifyContent='space-between'>
          ...
          >
            A decentralized social contract that outlines standards for on-chain and off-chain
            community behaviour.
          </GovernanceDB>
          <GovernanceDB
          ...
          >
            A token-weighted decision making tool, used for funds allocation, in which voting power
            is accrued as a function of tokens staked and time.
          </GovernanceDB>
          <GovernanceDB
            title='Tao Voting'
            button={
          ...
          >
            A token-weighted YES/NO decision making tool, with the option of delegation, that is
            used to make non-financial decisions in the GIVgarden.
          </GovernanceDB>
        </GovernanceRaw>
        <VoteCard>
          <H1>Vote in the GIVgarden</H1>
          <VoteCardDesc size='small'>
            The GIVgarden empowers the Giveth community to coordinate around shared resources from
            the bottom up.
          </VoteCardDesc>
          <VoteCardButton label='OPEN GIVGARDEN' buttonType='primary' onClick={goToGarden} />
        </VoteCard>
      </Container>
    </GardenBottomContainer>
  )
