...
export const TabGIVstreamTop = () => {
  ...
  useEffect(() => {
    setRewardLiquidPart(tokenDistroHelper.getLiquidPart(allocatedTokens.sub(givback)).sub(claimed))
    setRewardStream(tokenDistroHelper.getStreamPartTokenPerWeek(allocatedTokens.sub(givback)))
          ...
          <Row justifyContent='space-between'>
            <Left>
              ...
              <GSSubtitle size='medium'>
                Welcome to the expanding GIViverse! The GIVstream aligns community members with the
                long term success of Giveth and the GIVeconomy.
              </GSSubtitle>
            </Left>
            <Right>
              <GIVstreamRewardCard
                ...
                network={chainId}
                targetNetworks={[config.MAINNET_NETWORK_NUMBER, config.XDAI_NETWORK_NUMBER]}
              />
            </Right>
          </Row>
        </GIVstreamTopInnerContainer>
...
export const TabGIVstreamBottom = () => {
  ...
  const [streamAmount, setStreamAmount] = useState<BigNumber>(new BigNumber(0))
  const {
    currentValues: { balances }
  } = useSubgraph()
  const increaseSecRef = useRef<HTMLDivElement>(null)
  const supportedNetworks = [config.MAINNET_NETWORK_NUMBER, config.XDAI_NETWORK_NUMBER]

  useEffect(() => {
    setStreamAmount(
      tokenDistroHelper.getStreamPartTokenPerWeek(balances.allocatedTokens.sub(balances.givback))
    )
    ...
    <GIVbacksBottomContainer>
      <Container>
        ...
        <FlowRateRow alignItems='baseline' gap='8px'>
          ...
          <IconGIVStream size={64} />
          <H1>
            {chainId && supportedNetworks.includes(chainId) ? formatWeiHelper(streamAmount) : '0'}
          </H1>
          <FlowRateUnit>GIV/week</FlowRateUnit>
          <IconWithTooltip icon={<IconHelp size={16} />} direction={'top'}>
            <FlowRateTooltip>
              The rate at which you receive liquid GIV from your GIVstream.
            </FlowRateTooltip>
          </IconWithTooltip>
        </FlowRateRow>
        ...
        <Row wrap={1} justifyContent='space-between'>
          ...
          >
            Your GIVstream provides a continuous flow of claimable GIV until December 23, 2026.
            Anyone can get or increase their GIVstream by participating in the GIVeconomy.
          </GsDataBlock>
          <GsDataBlock title='Expanding GIViverse'>
            The GIVeconomy begins humbly but as time passes, the GIViverse expands and more GIV
            flows from GIVstream. This way, as the GIVeconomy grows, so do the governance rights of
            our community.
          </GsDataBlock>
        </Row>
        <HistoryTitleRow>
          ...
          <IconWithTooltip icon={<IconHelp size={16} />} direction={'top'}>
            <HistoryTooltip>
              Every time you claim GIV rewards from GIVbacks, the GIVgarden, or the GIVfarm, your
              GIVstream flowrate increases. Below is a summary.
            </HistoryTooltip>
          </IconWithTooltip>
        </HistoryTitleRow>
        <GIVstreamHistory />
      ...
      <IncreaseSection ref={increaseSecRef}>
        <Container>
          ...
          <Row wrap={1} justifyContent='space-between'>
            ...
            >
              Donate to verified projects on Giveth. Get GIV and increase your GIVstream with the
              GIVbacks program.
            </IGsDataBox>
            <IGsDataBox
              title='GIVgarden'
            ...
            >
              The GIVgarden is the decentralized governance platform for the GIVeconomy. Increase
            ...
            <IGsDataBox
              ...
              button={
                <Link href='/givfarm' passHref>
                  <GsButton label='SEE OPPORTUNITIES' linkType='primary' size='medium' />
                </Link>
              }
            >
              Stake GIV, or become a liquidity provider and stake LP tokens in the GIVfarm. Get GIV
              rewards and increase your GIVstream.
            </IGsDataBox>
          </Row>
        </Container>
      </IncreaseSection>
    ...
    <GIVstreamProgressContainer>
      <GsPTitleRow justifyContent='space-between'>
        <GsPTitle alignItems='center' gap='8px'>
          ...
          <IconWithTooltip icon={<IconHelp size={16} />} direction={'right'}>
            <GsPTooltip>
              Time left for all GIVstreams to flow & for the GIViverse to reach full power!
            </GsPTooltip>
          </IconWithTooltip>
        </GsPTitle>
...
export const GIVstreamHistory: FC = () => {
  ...
  const [tokenAllocations, setTokenAllocations] = useState<ITokenAllocation[]>([])
  ...
  useEffect(() => {
    if (chainId && account) {
      setLoading(true)
      getHistory(chainId, account, page * count, count).then(_tokenAllocations => {
  ...
  return (
    <HistoryContainer>
      ...
      {tokenAllocations && tokenAllocations.length > 0 && (
        <Grid>
          {tokenAllocations.map((tokenAllocation, idx) => {
            ...
            const date = d.toDateString().split(' ').splice(1, 3).join(' ')
            return (
              // <span key={idx}>1</span>
              <Fragment key={idx}>
                <P as='span'>
                  {convetSourceTypeToIcon(tokenAllocation.distributor || 'Unknown')}
                ...
                <B as='span'>
                  ...
                  {formatWeiHelper(
                    tokenDistroHelper.getStreamPartTokenPerWeek(
                      ethers.BigNumber.from(tokenAllocation.amount)
                    )
                  )}
      ...
      {(!tokenAllocations || tokenAllocations.length == 0) && <NoData> NO DATA</NoData>}
      {numberOfPages > 1 && (
        <PaginationRow justifyContent={'flex-end'} gap='16px'>
          ...
          <PaginationItem
            onClick={() => {
              if (page + 1 < numberOfPages) setPage(page => page + 1)
            }}
