...
export const USDInput: FC<IAmountInput> = ({ maxAmount, poolStakingConfig, disabled = false }) => {
  const [displayAmount, setDisplayAmount] = useState('')

  const setAmountPercentage = useCallback(
    (percentage: number): void => {
      const newAmount = BigNumber.from(maxAmount).mul(percentage).div(100).toString()
