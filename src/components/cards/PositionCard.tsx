...
const V3StakingCard: FC<IV3StakeCardProps> = ({
  ...
  return (
    <PositionContainer key={position.tokenId.toString()}>
      {image && (
        <ImageContainer>
          <Image src={image} width={72} height={124} alt='Liquidity Position NFT' />
        </ImageContainer>
      )}
      <PositionInfo>
        <PositionInfoRow>
          ...
          <RoundedInfo>
            {position._position ? position._position?.pool.fee / 10000 : '-'}%
          </RoundedInfo>
          <RoundedInfo>
            {inRange ? (
              <>
              ...
              >
                <RangeTooltip>
                  Your staked position is out of range and is not earning rewards.{' '}
                  <PositionLink
                    target='_blank'
        ...
        <TokenAmountRow>
          <IconGiveth size={16} />
          <TokenValue>
            {position._position ? position._position.amount0.toSignificant(4) : '-'}
          </TokenValue>
        </TokenAmountRow>
        <TokenAmountRow>
          <IconETH size={16} />
          <TokenValue>
            {position._position ? position._position.amount1.toSignificant(4) : '-'}
          </TokenValue>
        </TokenAmountRow>
      </PositionInfo>
      <PositionActions>
        ...
        <FullWidthButton
          label='VIEW POSITION'
          onClick={() =>
            window.open(`https://app.uniswap.org/#/pool/${position.tokenId.toString()}`)
          }
          buttonType='texty'
