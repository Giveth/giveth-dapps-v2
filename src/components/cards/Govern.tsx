...
const GovernCard: FC<IClaimViewCardProps> = ({ index }) => {
  ...
  useEffect(() => {
    ...
    const convertedStackedWithApr = EthersBigNumber.from(stackedWithApr.toFixed(0)).mul(
  ...
  return (
    <GovernCardContainer activeIndex={step} index={index}>
      <BeeImage>
        <Image src='/images/bee1.svg' height='81' width='112' alt='Image of a happy bee' />
      </BeeImage>
      <GovernHeader>
        <Title as='h1'>Engage in Governance</Title>

        <Desc>
          Participate in Giveth governance using the <b>GIVgarden</b>. Wrap GIV to vote on proposals
          and earn rewards.
        </Desc>
      </GovernHeader>
        ...
        <ImpactCard>
          ...
          <div>
            <Row alignItems={'center'} justifyContent={'space-between'}>
              <ImpactCardLabel>Amount of GIV wrapped</ImpactCardLabel>
              <MaxStakeGIV
                onClick={() => setStacked(utils.formatEther(totalAmount.div(10)))}
              >{`Max ${utils.formatEther(totalAmount.div(10))} GIV`}</MaxStakeGIV>
            </Row>
            <ImpactCardInput>
              <InputWithUnit type='number' value={stacked} unit={'GIV'} onChange={setStacked} />
            </ImpactCardInput>
          </div>
        </ImpactCard>
        <PoolCardContainer>
          ...
          <PoolCard>
            <PoolItems>
              <Row justifyContent='space-between'>
                ...
                <PoolItemBold>
                  <Image src='/images/icons/star.svg' height='16' width='16' alt='Star icon' />
