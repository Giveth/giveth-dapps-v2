...
const ClaimCardContainer = styled(Card)<IClaimCardContainer>`
  ::before {
    content: '';
    background-image: ${props => (props.claimed ? '' : 'url(/images/wave.png)')};
    position: absolute;
    height: 143px;
    top: 0;
    left: 0;
...
const ClaimCard: FC<IClaimViewCardProps> = ({ index }) => {
  ...
  return (
    <>
      <ClaimCardContainer activeIndex={step} index={index} claimed={txStatus}>
        <ClaimHeader>
          <Title as='h1' weight={700}>
            Claim your GIV now!
          </Title>
          <Desc>Let&apos;s Build the Future of Giving, together.</Desc>
        </ClaimHeader>
