...
import { Subline, neutralColors, IconHelp, H2, H5, Lead } from '@giveth/ui-design-system'
...
export const DonateCard: FC<IClaimViewCardProps> = ({ index }) => {
  ...
  const [potentialClaim, setPotentialClaim] = useState<EthersBigNumber>(constants.Zero)
  ...
  useEffect(() => {
    ...
    const convertedStackedWithApr = EthersBigNumber.from(donationWithGivBacks.toFixed(0)).mul(
      constants.WeiPerEther
    )
    setPotentialClaim(tokenDistroHelper.getLiquidPart(convertedStackedWithApr))
    setEarnEstimate(tokenDistroHelper.getStreamPartTokenPerWeek(convertedStackedWithApr))
  ...
  return (
    <DonateCardContainer activeIndex={step} index={index}>
      <DonateHeader>
        ...
        <Desc>
          Donate to verified projects to get GIV with <b>GIVbacks</b>. The project gets 100% of your
          donation, and you get rewarded by Giveth with GIV!
        </Desc>
      </DonateHeader>
            ...
            <Row justifyContent={'space-between'}>
              <Row gap='4px' alignItems='center'>
                ...
                <IconWithTooltip icon={<IconHelp size={16} />} direction={'top'}>
                  <GdropDonateTooltip>
                    Donations made in most tokens are eligible for GIVbacks.
                  </GdropDonateTooltip>
                </IconWithTooltip>
              </Row>
              <MaxStakeGIV
                onClick={() => setDonation(utils.formatEther(totalAmount.div(10)))}
              >{`Max ${utils.formatEther(totalAmount.div(10))} GIV`}</MaxStakeGIV>
            </Row>
            <ImpactCardInput>
              <InputWithUnit type='number' value={donation} unit={'GIV'} onChange={setDonation} />
            </ImpactCardInput>
          </div>
        </ImpactCard>
        <PoolCardContainer>
          <DonatePoolCard>
            <PoolItems>
              <Row justifyContent='space-between'>
                ...
                <PoolItemBold>{formatWeiHelper(potentialClaim)} GIV</PoolItemBold>
              </Row>
              <Row justifyContent='space-between'>
                <PoolItem>Streaming</PoolItem>
                <PoolItemBold>{formatWeiHelper(earnEstimate)} GIV/week</PoolItemBold>
              </Row>
            </PoolItems>
          </DonatePoolCard>
        </PoolCardContainer>
      ...
      <PoolCardFooter>
        These calculators demonstrate how you can use GIV to participate in the GIVeconomy!{' '}
        <b>These are just simulations.</b> To participate for real, claim your GIV.
      </PoolCardFooter>
      {step === index && (
        <>
