...
import { OutOfRangeBadgeContianer, OutOfRangeTooltip } from './BaseStakingCard.sc'
...
const StakingPositionCard: FC<IStakingPositionCardProps> = ({ network, poolStakingConfig }) => {
  const { rewardBalance } = useStakingNFT()
  const { apr, unstakedPositions, stakedPositions } = useLiquidityPositions()
  const [oneOfPositionsOutOfRange, setOneOfPositionsOutOfRange] = useState(false)
  ...
  useEffect(() => {
    const _oneOfPositionsOutOfRange = stakedPositions.some(stakedPosition => {
      const { pool, tickLower, tickUpper } = stakedPosition._position || {}
      // Check price range
      const below = pool && typeof tickLower === 'number' ? pool.tickCurrent < tickLower : undefined
      const above =
        pool && typeof tickUpper === 'number' ? pool.tickCurrent >= tickUpper : undefined
      return below || above
    })
    ...
    <BaseStakingCard
      ...
      notif={
        oneOfPositionsOutOfRange && (
          <IconWithTooltip icon={<OutOfRangeBadge />} direction={'top'}>
            <OutOfRangeTooltip>
              One or more of your position(s) are out of range and not earning rewards. Please
              unstake and make a new position.
            </OutOfRangeTooltip>
          </IconWithTooltip>
        )
      }
