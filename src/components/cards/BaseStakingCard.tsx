...
const BaseStakingCard: FC<IBaseStakingCardProps> = ({ stakeInfo, poolStakingConfig, notif }) => {
  ...
  const { type, title, description, provideLiquidityLink, BUY_LINK, unit } = poolStakingConfig
      ...
      <StakingPoolContainer>
        <StakingPoolExchangeRow gap='4px' alignItems='center'>
          ...
          <StakingPoolExchange styleType='Small'>
            {type === StakingType.GIV_LM && chainId === config.XDAI_NETWORK_NUMBER && `GIVgarden `}
            {type}
          </StakingPoolExchange>
          {chainId === config.XDAI_NETWORK_NUMBER && type === StakingType.GIV_LM && (
            <IconWithTooltip
              direction={'top'}
              icon={<IconHelp color={brandColors.deep[100]} size={12} />}
            >
              <GIVgardenTooltip>
                While staking GIV in this pool you are also granted voting power (gGIV) in the
                GIVgarden.
              </GIVgardenTooltip>
            </IconWithTooltip>
          )}
        ...
        <SPTitle alignItems='center' gap='16px'>
          ...
          <div>
            <StakingPoolLabel weight={900}>{title}</StakingPoolLabel>
        ...
        <StakePoolInfoContainer>
          <Details>
            <FirstDetail justifyContent='space-between'>
              ...
              <Row gap='8px' alignItems='center'>
                <IconSpark size={24} color={brandColors.mustard[500]} />
                <DetailValue>{apr && formatEthHelper(apr, 2)}%</DetailValue>
                <IconContainer onClick={() => setShowAPRModal(true)}>
                  <IconHelp size={16} />
                </IconContainer>
              </Row>
            </FirstDetail>
            <Detail justifyContent='space-between'>
              ...
              <DetailValue>{`${formatWeiHelper(rewardLiquidPart)} GIV`}</DetailValue>
            </Detail>
            <Detail justifyContent='space-between'>
              <Row gap='8px' alignItems='center'>
              ...
              <Row gap='4px' alignItems='center'>
                <DetailValue>{formatWeiHelper(rewardStream)}</DetailValue>
          ...
          <LiquidityButton
            label={type === StakingType.GIV_LM ? 'BUY GIV TOKENS' : 'PROVIDE LIQUIDITY'}
            onClick={() =>
              window.open(type === StakingType.GIV_LM ? BUY_LINK : provideLiquidityLink)
            }
            buttonType='texty'
            icon={<IconExternalLink size={16} color={brandColors.deep[100]} />}
          />
        </StakePoolInfoContainer>
      </StakingPoolContainer>
      {showAPRModal && (
