import styled from 'styled-components';
import Image from 'next/image';
import CausesMainCategories from '@/components/views/causes/filter/CausesMainCategories';
import { mediaQueries } from '@/lib/constants/constants';
import { IconContainer } from '@/components/views/projects/common.styled';

const CausesFiltersSwiper = () => {
	return (
		<Container className='fadeIn'>
			<PrevIcon id='prevIcon'>
				<Image
					src={'/images/caret_right.svg'}
					alt='caret right'
					width={5}
					height={10}
				/>
			</PrevIcon>
			<CausesMainCategories />
			<NextIcon id='nextIcon'>
				<Image
					src={'/images/caret_right.svg'}
					alt='caret right'
					width={5}
					height={10}
				/>{' '}
			</NextIcon>
		</Container>
	);
};

const Container = styled.div`
	display: flex;
	align-items: center;
	position: relative;
	gap: 16px;
	width: 100%;
	${mediaQueries.tablet} {
		max-width: 90%;
	}
	${mediaQueries.laptopS} {
		padding-right: 60px;
		width: 70%;
	}
`;

const NextIcon = styled(IconContainer)`
	z-index: 1;
	display: none;
	:disabled {
		opacity: 0.5;
		cursor: default;
	}
	${mediaQueries.tablet} {
		display: inline-block;
	}
	${mediaQueries.laptopS} {
		display: block;
		min-width: 50px;
		min-height: 50px;
		right: 0;
	}
`;

const PrevIcon = styled(NextIcon)<{ disabled?: boolean }>`
	-ms-transform: rotate(180deg);
	transform: rotate(180deg);
	left: 0;
	z-index: 2;
	${mediaQueries.laptopS} {
	}
`;

export default CausesFiltersSwiper;
