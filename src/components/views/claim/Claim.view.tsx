...
const Step: FC<IStepProps> = ({ title, isActive, onClick, disabled }) => {
  return (
    <StepTitle isActive={isActive} onClick={disabled ? undefined : onClick} disabled={disabled}>
      {title}
    </StepTitle>
  )
}
...
const ClaimView = () => {
  ...
  return (
    <>
      ...
      {step < 6 ? (
        <ClaimViewContainer switchNetwork={chainId === config.XDAI_NETWORK_NUMBER || chainId === 0}>
