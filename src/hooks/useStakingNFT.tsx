...
export const useStakingNFT = () => {
  ...
  const [rewardBalance, setRewardBalance] = useState<BigNumber>(BigNumber.from(0))
  ...
  }, [rewardToken, poolAddress, incentiveRefundeeAddress, chainId, uniswapConfig])
      ...
      try {
        const rewards = await Promise.all(
          stakedPositions.map(({ tokenId }) =>
            getReward(tokenId, uniswapV3StakerContract, currentIncentive.key)
          )
        )
  ...
  useEffect(() => {
    if (!account || !currentIncentive.key || chainId !== config.MAINNET_NETWORK_NUMBER) return
  ...
  }, [account, chainId, checkForRewards, currentIncentive.key, stakedPositions])
