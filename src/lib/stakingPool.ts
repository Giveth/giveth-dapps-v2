...
const toBigNumber = (eb: ethers.BigNumber): BigNumber => new BigNumber(eb.toString())
...
export const getGivStakingAPR = async (
  ...
  if (unipool) {
    ...
    apr = totalSupply.isZero()
      ? Zero
      : toBigNumber(rewardRate).div(totalSupply.toString()).times('31536000').times('100')
  }
...
export const getLPStakingAPR = async (
  ...
  if (poolStakingConfig.type === StakingType.BALANCER) {
    ...
    return getSimplePoolStakingAPR(poolStakingConfig, network, provider, unipool)
  }
}
...
const getBalancerPoolStakingAPR = async (
  ...
  const { LM_ADDRESS, POOL_ADDRESS, VAULT_ADDRESS, POOL_ID } = balancerPoolStakingConfig
  const tokenAddress = config.NETWORKS_CONFIG[network].TOKEN_ADDRESS

  const lmContract = new Contract(LM_ADDRESS, LM_ABI, provider)
  const poolContract = new Contract(POOL_ADDRESS, BAL_WEIGHTED_POOL_ABI, provider)
  ...
  try {
    ...
    apr = totalSupply.isZero()
      ? null
      : toBigNumber(rewardRate).div(totalSupply.toString()).times('31536000').times('100').times(lp)
  } catch (e) {
...
const getSimplePoolStakingAPR = async (
  ...
  try {
    ...
    if (_token0.toLowerCase() !== tokenAddress.toLowerCase()) reserves.reverse()
    const lp = toBigNumber(_poolTotalSupply)
      .times(10 ** 18)
      .div(2)
      .div(reserves[0])
...
export const getUserStakeInfo = (
  ...
  if (unipoolHelper) {
    earned = unipoolHelper.earned(rewards, rewardPerTokenPaid, stakedAmount)
  }
...
export const approveERC20tokenTransfer = async (
  ...
  const allowance: BigNumber = await tokenContract.allowance(owenerAddress, spenderAddress)
  ...
  const gasPreference = getGasPreference(config.NETWORKS_CONFIG[provider.network.chainId])
...
export const wrapToken = async (
  ...
  const gardenContract = new Contract(gardenAddress, TOKEN_MANAGER_ABI, signer)
  try {
    return await gardenContract
      .connect(signer.connectUnchecked())
      .wrap(amount, getGasPreference(config.NETWORKS_CONFIG[provider.network.chainId]))
  } catch (error) {
...
export const unwrapToken = async (
  ...
  try {
    return await gardenContract
      .connect(signer.connectUnchecked())
      .unwrap(amount, getGasPreference(config.NETWORKS_CONFIG[provider.network.chainId]))
  } catch (error) {
...
export const stakeTokens = async (
  ...
  const rawPermitCall = await permitTokens(provider, poolAddress, lmAddress, amount)
  ...
  try {
    ...
    return await lmContract
      .connect(signer.connectUnchecked())
      .stakeWithPermit(ethers.BigNumber.from(amount), rawPermitCall.data, {
        gasLimit: 300_000,
        ...gasPreference
      })
  } catch (e) {
...
export const harvestTokens = async (
  ...
  const signer = provider.getSigner()
  const lmContract = new Contract(lmAddress, LM_ABI, signer.connectUnchecked())
