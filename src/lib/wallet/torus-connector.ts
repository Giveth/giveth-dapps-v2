...
export class TorusConnector extends AbstractConnector {
  ...
  constructor({ chainIds, constructorOptions = {}, loginOptions = {} }: TorusConnectorArguments) {
  ...
  public async activate(): Promise<ConnectorUpdate> {
    if (!this.torus) {
      const TorusEmbed = await import('@toruslabs/torus-embed').then(m => m?.default ?? m)

      this.torus = new TorusEmbed(this.constructorOptions)

      const host = networks[this.chainIds[0]] || networks[config.MAINNET_NETWORK_NUMBER]
  ...
  public async getAccount(): Promise<null | string> {
    const accounts: Maybe<[string]> | undefined = await this?.torus?.ethereum?.request({
      method: 'eth_accounts'
    })
  ...
  public async changeChainId(network: number) {
    const host = networks[network] || networks[config.MAINNET_NETWORK_NUMBER]
