...
export function showPendingStake(amount: string, network: number, txHash: string): void {
  const transactionExplorer = `${networksParams[network].blockExplorerUrls[0]}/tx/${txHash}`

  toast.success(
    <span>
      You are staking {amount} tokens. Check the status of your transaction{' '}
      <a target='_blank' href={transactionExplorer} rel='noreferrer' style={{ color: 'white' }}>
        here
      </a>
      .
    </span>
