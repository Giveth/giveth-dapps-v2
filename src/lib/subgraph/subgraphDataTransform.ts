...
const transformBalanceInfo = (info: any): IBalances => {
  ...
  const rewardPerTokenPaidSushiSwap = BN(info.rewardPerTokenPaidSushiSwap || 0)
  const rewardsSushiSwap = BN(info.rewardsSushiSwap || 0)
  const rewardPerTokenPaidHoneyswap = BN(info.rewardPerTokenPaidHoneyswap || 0)
...
const transformUniswapPositions = (info: any): any => {
  ...
  return {
    userStakedPositions: userStakedPositions ? userStakedPositions.map(mapper) : [],
    userNotStakedPositions: userNotStakedPositions ? userNotStakedPositions.map(mapper) : [],
...
export const transformSubgraphData = async (data: any = {}): Promise<ISubgraphValue> => {
  return {
    ...
    [StakingType.BALANCER]: transformUnipoolInfo(data[StakingType.BALANCER]),
    [StakingType.HONEYSWAP]: transformUnipoolInfo(data[StakingType.HONEYSWAP]),
    [StakingType.SUSHISWAP]: transformUnipoolInfo(data[StakingType.SUSHISWAP]),
