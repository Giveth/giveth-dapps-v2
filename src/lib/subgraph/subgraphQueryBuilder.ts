import { SimplePoolStakingConfig, StakingType, UniswapV3PoolStakingConfig } from '@/types/config'
...
export class SubgraphQueryBuilder {
  ...
  private static generateUnipoolInfoQueries = (configs: SimplePoolStakingConfig[]): string => {
    return configs
      .map(c => `${c.type}: ${SubgraphQueryBuilder.getUnipoolInfoQuery(c.LM_ADDRESS)}`)
      .join()
  }
  ...
  static getMainnetQuery = (address: string): string => {
    ...
    return `
        ...
        ...config.MAINNET_CONFIG.pools.filter(c => c.type !== StakingType.UNISWAP)
      ])}
			uniswapV3Pool: ${SubgraphQueryBuilder.getUniswapV3PoolQuery(uniswapConfig.UNISWAP_V3_LP_POOL)}
  ...
  static getXDaiQuery = (address: string): string => {
    return `
        ...
        config.XDAI_CONFIG.pools.find(c => c.type === StakingType.SUSHISWAP)?.POOL_ADDRESS || ''
      )}
		}
		`
  }
