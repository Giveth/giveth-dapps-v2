...
export class TokenDistroHelper {
  ...
  get globallyClaimableNow(): ethers.BigNumber {
    ...
    const releasedAmount = new BigNumber(this.lockedAmount.toString())
      .times(deltaTime)
      .div(this.duration)
    return this.initialAmount.add(releasedAmount.toFixed(0, BigNumber.ROUND_DOWN))
  }
  ...
  public getStreamPartTokenPerSecond = (amount: ethers.BigNumber): BigNumber => {
  ...
  public getStreamPartTokenPerWeek = (amount: ethers.BigNumber): BigNumber => {
  ...
  public getUserClaimableNow(userBalance: IBalances): ethers.BigNumber {
    return this.getLiquidPart(userBalance.allocatedTokens).sub(userBalance.claimed)
  }

  public get GlobalReleasePercentage(): number {
    if (this.totalTokens.isZero()) return 0
    const initialPercent = this.getLiquidPart(this.totalTokens).mul(100).div(this.totalTokens)
