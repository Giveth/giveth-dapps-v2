...
export const TokenDistroProvider: FC = ({ children }) => {
  ...
  const [mainnetTokenDistro, setMainnetTokenDistro] =
    useState<TokenDistroHelper>(defaultTokenDistroHelper)
  const [xDaiTokenDistro, setXDaiTokenDistro] =
    useState<TokenDistroHelper>(defaultTokenDistroHelper)

  useEffect(() => {
    if (mainnetValues?.tokenDistroInfo)
      setMainnetTokenDistro(new TokenDistroHelper(mainnetValues.tokenDistroInfo))
  }, [mainnetValues])

  useEffect(() => {
    if (xDaiValues.tokenDistroInfo)
      setXDaiTokenDistro(new TokenDistroHelper(xDaiValues.tokenDistroInfo))
  }, [xDaiValues])
