...
export const FarmProvider: FC = ({ children }) => {
  ...
  const [xDaiTotalEarned, setxDaiTotalEarned] = useState(ethers.constants.Zero)
  const [mainnetInfos, setMainnetInfos] = useState<IInfos>({})
  const [mainnetTotalEarned, setMainnetTotalEarned] = useState(ethers.constants.Zero)
  const { account, chainId } = useWeb3React()

  const setInfo = useCallback((network: number, key: string, value: ethers.BigNumber) => {
  ...
  return (
    <FarmContext.Provider
      value={{
        totalEarned:
          chainId === config.MAINNET_NETWORK_NUMBER ? mainnetTotalEarned : xDaiTotalEarned,
        setInfo
      }}
    >
      {children}
