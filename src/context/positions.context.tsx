...
export const NftsProvider: FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const { userStakedPositions, userNotStakedPositions, allPositions, uniswapV3Pool } = mainnetValues

  const [stakedPositions, setStakedPositions] = useState<LiquidityPosition[]>([])
  const [unstakedPositions, setUnstakedPositions] = useState<LiquidityPosition[]>([])
  ...
  }, [rewardToken, poolAddress, incentiveRefundeeAddress, network, uniswapConfig])

  const transformToLiquidityPosition = useCallback(
    (
      { liquidity, owner, staked, tickLower, tickUpper, tokenId }: IUniswapV3Position,
      pool: Pool
  ...
  useEffect(() => {
    const loadPositions = async () => {
      try {
        ...
        const { TOKEN_ADDRESS, WETH_TOKEN_ADDRESS } = config.MAINNET_CONFIG

        const givToken = new Token(network, TOKEN_ADDRESS, 18, 'GIV', 'GIV')
        const wethToken = new Token(network, WETH_TOKEN_ADDRESS, 18, 'WETH', 'WETH')

        const givIsToken0 = uniswapV3Pool.token0.toLowerCase() === TOKEN_ADDRESS.toLowerCase()
        ...
        const allUserPositions: LiquidityPosition[] = (
          await Promise.all(
            [...userStakedPositions, ...userNotStakedPositions].map(positionInfo =>
              transformToLiquidityPosition(positionInfo, _pool)
            )
          )
        ).filter(p => p) as LiquidityPosition[]

        const downloadURI = async (position: LiquidityPosition): Promise<any | null> => {
          ...
          if (!uri) {
            try {
              ...
              console.error('Error on fetching uri of token ' + tokenId, e)
            }
          }
        ...
        const stakedPositions = allUserPositions.flat().filter(position => position.staked)

        const stakedPositionsWithURI = await Promise.all(stakedPositions.map(downloadURI))

        const unstakedPositions = allUserPositions.flat().filter(position => !position.staked)

        const unstakedPositionsWithURI = await Promise.all(unstakedPositions.map(downloadURI))
        ...
        if (
          ...
          const totalETHValue = allLiquidityPositions.flat().reduce((acc, { _position }) => {
            ...
            if (givIsToken0) {
              ...
              const ethValueGIV = _position.pool.token0Price.quote(givAmount)
              ...
              // calc value of GIV in terms of ETH
              const ethValueGIV = _position.pool.token1Price.quote(givAmount)
    ...
    loadPositions()
  }, [userNotStakedPositions, userStakedPositions, allPositions, uniswapV3Pool])
